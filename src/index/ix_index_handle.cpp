...
void IxNodeHandle::insert_pairs(int pos, const char *key, const Rid *rid, int n) {
    ...
    int tail_num = get_size() - pos;
    if (tail_num > 0) {
        memmove(get_key(pos + n), get_key(pos), static_cast<size_t>(tail_num) * key_len);
        memmove(get_rid(pos + n), get_rid(pos), static_cast<size_t>(tail_num) * sizeof(Rid));
    }
    memcpy(get_key(pos), key, static_cast<size_t>(n) * key_len);
    memcpy(get_rid(pos), rid, static_cast<size_t>(n) * sizeof(Rid));
    set_size(get_size() + n);
}
...
void IxNodeHandle::erase_pair(int pos) {
    assert(pos >= 0 && pos < get_size());
    int tail_num = get_size() - pos - 1;
    if (tail_num > 0) {
        memmove(get_key(pos), get_key(pos + 1), static_cast<size_t>(tail_num) * file_hdr->col_tot_len_);
        memmove(get_rid(pos), get_rid(pos + 1), static_cast<size_t>(tail_num) * sizeof(Rid));
    }
    set_size(get_size() - 1);
}
...
IxNodeHandle *IxIndexHandle::split(IxNodeHandle *node) {
    ...
    int split_pos = total_size / 2;
    int move_count = total_size - split_pos;
...
bool IxIndexHandle::delete_entry(const char *key, Transaction *transaction) {
    ...
    if (!leaf->is_root_page() && leaf->get_size() > 0) {
...
bool IxIndexHandle::coalesce_or_redistribute(IxNodeHandle *node, Transaction *transaction, bool *root_is_latched) {
    ...
    IxNodeHandle *neighbor =
        fetch_node(index == 0 ? parent->value_at(1) : parent->value_at(index - 1));
...
void IxIndexHandle::redistribute(IxNodeHandle *neighbor_node, IxNodeHandle *node, IxNodeHandle *parent, int index) {
    ...
    memcpy(parent->get_key(node_idx), node->get_key(0), file_hdr_->col_tot_len_);
    int neighbor_idx = parent->find_child(neighbor_node);
    memcpy(parent->get_key(neighbor_idx), neighbor_node->get_key(0), file_hdr_->col_tot_len_);
...
bool IxIndexHandle::coalesce(IxNodeHandle **neighbor_node, IxNodeHandle **node, IxNodeHandle **parent, int index,
    ...
    int old_size = (*neighbor_node)->get_size();
    (*neighbor_node)
        ->insert_pairs(old_size, (*node)->get_key(0), (*node)->get_rid(0), (*node)->get_size());
    ...
    if ((*neighbor_node)->is_leaf_page()) {
        ...
        for (int i = old_size; i < (*neighbor_node)->get_size(); ++i) {
    ...
    if ((*parent)->get_size() > 0) {
        int left_idx = (*parent)->find_child(*neighbor_node);
        memcpy((*parent)->get_key(left_idx), (*neighbor_node)->get_key(0), file_hdr_->col_tot_len_);
        maintain_parent(*parent);
    }
...
Iid IxIndexHandle::upper_bound(const char *key) {
    ...
    int slot_no = leaf->lower_bound(key);
    if (slot_no < leaf->get_size() &&
        ix_compare(leaf->get_key(slot_no), key, file_hdr_->col_types_, file_hdr_->col_lens_) == 0) {
        slot_no++;
    }
